...
function recordInteraction(description: string) {
  stepCounter++;
...
async function launchBrowserWithCapture() {
  try {
    ...
    await page.exposeFunction('recordHover', (selector: string) => {
      recordInteraction("Hovered over " + selector);
    });

    await page.exposeFunction('recordScroll', (direction: string, amount: number) => {
    ...
    await page.addInitScript(() => {
      ...
      document.addEventListener('input', (e) => {
        ...
        if (target.value) {
          ...
          (window as any).recordInput?.(selector, target.value);
        }
      }, true);
      ...
      document.addEventListener('mouseover', (e) => {
        ...
        if (['button', 'a', 'input'].includes(tagName)) {
          ...
          (window as any).recordHover?.(selector);
        }
      }, true);
      ...
      window.addEventListener('scroll', () => {
        ...
        if (amount > 50) { // Only record significant scrolls
          (window as any).recordScroll?.(direction, amount);
